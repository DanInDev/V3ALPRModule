...
describe('filterWithMultipleOptions function', () => {
  ...
  it('should return nothing, as there is no Romanian Number Plates in this test', () => {
    const filteredBlocks = filterWithMultipleOptions(
      mockOCRFrame,
      romanianCollection,
    );
    expect(filteredBlocks).toBeNull; // Nothing should pass through, as there is no Romanian License Plates
  });
});
